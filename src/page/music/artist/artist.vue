<template>
  ...
  <div
    class="grid grid-flow-row grid-cols-4 miniplayer:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10 gap-5"
  >
    <div
...
import { getArtistList } from '@/service/modules/artist';
...
const getData = async () => {
  ...
  try {
    const res = await getArtistList(pageData);
    if (pageData.page === 1) {
      artists.value = res;
    } else {
...
const optionChange = (keyName: string, keyValue: number | string) => {
  pageData.page = 1;
  if (keyName === 'area') {
    pageData.area = keyValue as number;
  }
  if (keyName === 'type') {
    pageData.type = keyValue as number;
  }
  if (keyName === 'initial') {
    pageData.initial = keyValue as string;
  }
  getData();
};
